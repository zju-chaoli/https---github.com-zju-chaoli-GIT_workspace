...
void dijkstra(GRAPH G[], int size);
...
void print(GRAPH G[], int size)
{
	...
	for(i=0;i<size;i++)
	{
		printf("G[%d|indegree=%d|dist=%d|element=%d]-->[", i,G[i].indegree,G[i].dist,G[i].element);
		tmp=G[i].child;
		while(tmp!=NULL)
		{
...
void topSort_2(GRAPH G[], int size)
{
	...
	int j=0;
	VEDEX *T=NULL;
	//enqueue all indegree which equal to 0
	for(i=0;i<size;i++)
		if(G[i].indegree==0)
		{
...
void dijkstra(GRAPH G[], int size)
{
	...
	while(!isEmpty(Queue,size))
	{
		...
		while(T!=NULL)
		{
			...
			if(G[T->value].dist>G[i].dist+T->weight)
			{
